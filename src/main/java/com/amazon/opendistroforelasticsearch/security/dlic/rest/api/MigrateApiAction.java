...
public class MigrateApiAction extends AbstractApiAction {
    ...
    public MigrateApiAction(final Settings settings, final Path configPath, final RestController controller, final Client client,
        ...
        controller.registerHandler(Method.POST, "/_opendistro_security/api/migrate", this);
    }
    ...
    protected void handlePost(RestChannel channel, RestRequest request, Client client, final JsonNode content) throws IOException {
        ...
        final Builder searchGuardIndexSettings = Settings.builder();

        if (autoExpandReplicas == null) {
            searchGuardIndexSettings.put(IndexMetaData.SETTING_NUMBER_OF_REPLICAS, replicas);
        } else {
            searchGuardIndexSettings.put(IndexMetaData.SETTING_AUTO_EXPAND_REPLICAS, autoExpandReplicas);
        }

        searchGuardIndexSettings.put(IndexMetaData.SETTING_NUMBER_OF_SHARDS, 1);
        ...
        client.admin().indices().prepareDelete(this.opendistroIndex).execute(new ActionListener<AcknowledgedResponse>() {
            ...
            public void onResponse(AcknowledgedResponse response) {
                ...
                if (response.isAcknowledged()) {
                    ...
                    client.admin().indices().prepareCreate(opendistroIndex).setSettings(searchGuardIndexSettings)
